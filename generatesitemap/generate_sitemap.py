...
class Generator():
    db = None
    def __init__(self, domain, db_host, db_name, db_user, db_password, url_prefix, xml_location):
        self.domain = domain
    ...
    def get_latest_generated_id(self):
        db = self.db_()
        latest = db.table('sitemap_generated').order_by('id',  'desc').first()
        if latest:
    ...
    def generate_xml(self):
        ...
        for articles in self.db_().table('article').where('id', '>', latest_id).order_by('id', 'desc').chunk(100):
            for item in articles:
                # delete fist one then append new one
                listed_url = xml_root.findall('url')
                if listed_url and len(listed_url) > 100:
                    xml_root.remove(listed_url[0])
